    ...
    def train_metatransfer_step(self,
        ...
        for _ in range(meta_batch_size):
            ...
            for batch in _mini_batches(mini_dataset, inner_batch_size, inner_iters, replacement):
                ...
                if self._pre_step_op:
                    self.session.run(self._pre_step_op)
                self.session.run(minimize_op_metalearner, feed_dict={input_ph: inputs, label_ph: labels, real_label: real_labels})
            new_vars_meta.append(self._model_state.export_variables())
            self._model_state.import_variables(old_vars)
            ...
            self._model_state.import_variables(old_vars)
        new_vars_meta = average_vars(new_vars_meta)
        new_vars_classifier = average_vars(new_vars_classifier)
        metaleaner_param_new = subtract_vars(old_vars,scale_vars(subtract_vars(old_vars, interpolate_vars(old_vars, new_vars_meta, meta_step_size)),1-beta))
